...
var api_key  = require('../api.json').key;
...
describe('TESTS SCRAPER UTILITY', function() {
  ...
  it('#should test Scraper.wrapDOI', function() {
    var self = this;
    var wrapped = 'http://alm.plos.org/api/v3/articles?api_key={key}&ids={id}'.replace('{key}', api_key); 
...
describe('TESTS API-REQUESTS', function() {
  ...
  before(function() {
    ...
    this.url = 'http://api.plos.org/search?q="smartphone"&api_key={key}'.replace('{key}', api_key);
  });
