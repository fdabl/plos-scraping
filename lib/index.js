...
var Scraper = function(config) {
  ...
  this.fulltext   = config.fulltext || false;
  this.meta       = config.meta || ['author_display', 'title_display', 'abstract', 
                                    'publication_date', 'article_type', 'journal']
  this.directory  = config.directory || './results';
  this.altmetrics = config.altmetrics || ['citations', 'views', 'shares', 'bookmarks'];
  ...
  ********************************************/
  ...
  this.scrapeText = function(url, cb) {
    request(url, function(err, res, body) {
      if (err) return cb(err);
      var $ = cheerio.load(body);
      var fulltext = $('#section1, #section2, #section3, #section4 > p').text();
      cb(null, fulltext);
    });
  };  


  this.avoidParseError = function(body) {
    var body = body.substr(body.indexOf('['));
  ...
  this.getAltmetrics = function(articles, callback) {
    ...
    async.map(urls, function(url, cb) {
      request(url, function(err, res, body) {
        var body = self.avoidParseError(body);
        ...
        if (self.fulltext) {
          self.scrapeText(altjson.url, function(err, fulltext) {
            if (err) return cb(err);

            specifics['fulltext'] = fulltext;
